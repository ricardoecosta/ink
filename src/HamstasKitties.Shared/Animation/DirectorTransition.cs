...
namespace HamstasKitties.Animation
{
    public abstract class DirectorTransition
    {
        ...
        /// </summary>
        public UI.Texture LoadingTexture { get; set; }

        private object isInLoadingScreenModeLock = new object();

        public bool CompareAndSetInLoadingScreenMode(bool compareValue, bool valueToSetIfComparisonSucceeds)
        {
            lock (this.isInLoadingScreenModeLock)
            {
                if (this.isInLoadingScreenMode == compareValue)
                {
                    this.isInLoadingScreenMode = valueToSetIfComparisonSucceeds;
                    return true;
                }
                else
                {
