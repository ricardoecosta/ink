...
namespace HamstasKitties.Mechanics.Emitters
{
    ...
    /// </summary>
    public abstract class BlockEmitter : HamstasKitties.UI.IUpdateable
    {
        ...
        {
            CurrentLineEmissionIntervalInSeconds = GetDataFromDictionary<float>(State.CurrentStateSettings, PersistableSettingsConstants.LineEmissionIntervalKey, GlobalConstants.InitialLineEmissionIntervalInSeconds);
            TotalDroppedLines = GetDataFromDictionary<int>(State.CurrentStateSettings, PersistableSettingsConstants.TotalDroppedLinesKey, 0);
            NumberOfKittiesToCreateInNextLineOfBlocks = GetDataFromDictionary<int>(State.CurrentStateSettings, PersistableSettingsConstants.NumberOfKittiesToCreateInNextLineOfBlocksKey, NumberOfKittiesToCreateInNextLineOfBlocks);
            KittiesProbability = GetDataFromDictionary<double>(State.CurrentStateSettings, PersistableSettingsConstants.KittiesProbabilityKey, KittiesProbability);
            List<BlockState> savedBlocks = GetDataFromDictionary<List<BlockState>>(State.CurrentStateSettings, PersistableSettingsConstants.LineOfBlocksKey, null);
