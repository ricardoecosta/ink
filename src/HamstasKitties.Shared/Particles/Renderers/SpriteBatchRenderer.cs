...
namespace HamstasKitties.Particles.Renderers
{
    using System;
    ...
    public sealed class SpriteBatchRenderer : Renderer
    {
        ...
        public void RenderEmitter(Emitter emitter, SpriteBatch spriteBatch)
        {
            ...
            if (emitter.ParticleTexture != null && emitter.ActiveParticlesCount > 0)
            {
                ...
                for (int i = 0; i < emitter.ActiveParticlesCount; i++)
                {
                    float scale = emitter.Particles[i].Scale / emitter.ParticleTexture.Width;
                    spriteBatch.Draw(emitter.ParticleTexture, emitter.Particles[i].Position, source, new Color(emitter.Particles[i].Colour), emitter.Particles[i].Rotation, origin, scale, SpriteEffects.None, 0f);
                }
            }
        }
        ...
        private BlendState GetBlendState(EmitterBlendMode emitterBlendMode)
        {
            switch (emitterBlendMode)
            {
                ...
                default:
                    {
                        // FIXME throw new InvalidEnumArgumentException("emitterBlendMode", (int)emitterBlendMode, typeof(EmitterBlendMode));
                        throw new Exception("InvalidEnumArgumentException: emitterBlendMode = " + (int)emitterBlendMode);
                    }
            }
        }
    }
