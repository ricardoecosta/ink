    ...
    public sealed class RadialGravityModifier : Modifier
    {
        ...
        protected internal override void Process(float dt, Particle[] particleArray, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    X = this.Position.X - particleArray[i].Position.X,
                    Y = this.Position.Y - particleArray[i].Position.Y
                };
                ...
                if (squareDistance < this.SquareRadius && squareDistance > this.SquareInnerRadius)
                {
                    ...
                    particleArray[i].Velocity.X += force.X;
                    particleArray[i].Velocity.Y += force.Y;
                }
            }
        }
    }
