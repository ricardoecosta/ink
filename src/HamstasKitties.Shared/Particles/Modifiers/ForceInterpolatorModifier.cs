    ...
    public class ForceInterpolatorModifier : Modifier
    {
        ...
        protected internal override void Process(float dt, Particle[] particleArray, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    particleArray[i].Velocity.X = previousParticle.Velocity.X;
                    particleArray[i].Velocity.Y = previousParticle.Velocity.Y;
                ...
                {
                    float positionDelta = (particleArray[i].Age / this.MiddlePosition);
                    particleArray[i].Velocity.X += this.InitialForce.X + ((this.MiddleForce.X - this.InitialForce.X) * positionDelta);
                    particleArray[i].Velocity.Y += this.InitialForce.Y + ((this.MiddleForce.Y - this.InitialForce.Y) * positionDelta);

                }
                else
                {
                    float positionDelta = ((particleArray[i].Age - this.MiddlePosition) / (1f - this.MiddlePosition));
                    particleArray[i].Velocity.X += this.MiddleForce.X + ((this.FinalForce.X - this.MiddleForce.X) * positionDelta);
                    particleArray[i].Velocity.Y += this.MiddleForce.Y + ((this.FinalForce.Y - this.MiddleForce.Y) * positionDelta);
                }
