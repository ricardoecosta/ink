    ...
    public class OpacityInterpolatorModifier : Modifier
    {
        ...
        protected internal override void Process(float dt, Particle[] particleArray, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    particleArray[i].Colour.W = previousParticle.Colour.W;

                    continue;
                }

                if (particleArray[i].Age < this.MiddlePosition)
                    particleArray[i].Colour.W = this.InitialOpacity + ((this.MiddleOpacity - this.InitialOpacity) * (particleArray[i].Age / this.MiddlePosition));

                else
                    particleArray[i].Colour.W = this.MiddleOpacity + ((this.FinalOpacity - this.MiddleOpacity) * ((particleArray[i].Age - this.MiddlePosition) / (1f - this.MiddlePosition)));
