...
namespace HamstasKitties.Particles.Modifiers
{
    ...
    public class OpacityOscillator : Modifier
    {
        ...
        protected internal override void Process(float dt, Particle[] particleArray, int count)
        {
            ...
            {
                float secondsAlive = this.TotalSeconds - particleArray[i].Inception;

                float sin = Calculator.Sin(secondsAlive * (this.Frequency * 3f));

                particleArray[i].Colour.W = ((this.MaximumOpacity - this.MinimumOpacity) * sin) + this.MinimumOpacity;
            }
        }
    }
}
