...
namespace HamstasKitties.Particles.Emitters
{
    using System;
    ...
    using HamstasKitties.Particles.Modifiers;
    using System.IO;
    ...
    public class Emitter
    {
        ...
        protected void CopyBaseFields(Emitter emitter)
        {
            ...
            emitter.ParticleTextureAssetName = this.ParticleTextureAssetName ?? String.Empty;
        ...
        /*private Particle[] RetireParticles(Particle[] particleArray, int count)
        {
            // FIXME: Not sure.

            Particle[] arrCopy = new Particle[particleArray.Length - count];
            Array.Copy(particleArray, count - 1, arrCopy, 0, particleArray.Length - count);
        ...
        public void Update(float deltaSeconds)
        {
            ...
            while (--i >= 0)
            {
                float actualAge = this.TotalSeconds - this.Particles[i].Inception;
                ...
                this.Particles[i].Age = actualAge / this.Term;

                this.Particles[i].Momentum.X += this.Particles[i].Velocity.X;
                this.Particles[i].Momentum.Y += this.Particles[i].Velocity.Y;

                this.Particles[i].Velocity.X = this.Particles[i].Velocity.Y = 0f;

                this.Particles[i].Position.X += this.Particles[i].Momentum.X * deltaSeconds;
                this.Particles[i].Position.Y += this.Particles[i].Momentum.Y * deltaSeconds;
            }

            // Retire particles if necessary...
            if (i >= 0)
                this.RetireParticles(i + 1);

            // Send the particle array to the modifiers for processing...
            this.Modifiers.RunProcessors(deltaSeconds, this.Particles, this.ActiveParticlesCount);
        }
        ...
        public void Trigger(ref Vector2 triggerPosition)
        {
            ...
            for (int i = oldIdle; i < oldIdle + this.ReleaseQuantity; i++)
            {
                if (i < this.Budget)
                {
                    ...
                    this.Particles[i].Inception = this.TotalSeconds;
                    this.Particles[i].Position.X = position.X + offset.X;
                    this.Particles[i].Position.Y = position.Y + offset.Y;
                    this.Particles[i].Velocity.X = force.X * speed;
                    this.Particles[i].Velocity.Y = force.Y * speed;
                    this.Particles[i].Momentum = this.ReleaseImpulse;
                    this.Particles[i].Age = 0f;
                    this.Particles[i].Colour = new Vector4(this.ReleaseColour.Sample(), this.ReleaseOpacity.Sample());
                    this.Particles[i].Scale = this.ReleaseScale.Sample();
                    this.Particles[i].Rotation = 0f;

                    this.Particles[i].Rotate(this.ReleaseRotation.Sample());
#if MPE_RAISEEVENTS
