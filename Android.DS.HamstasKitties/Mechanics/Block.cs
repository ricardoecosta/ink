    ...
    public class Block : LayerObject
    {
        ...
        public void UpgradeToSpecialType()
        {
            ...
            switch (CurrentSpecialType)
            {
...
#if WINDOWS_PHONE
                    BigExplosionParticleEffect = GameDirector.Instance.CurrentResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.BigExplosion, true);
                    BigExplosionParticleEffect.Initialise();
                    ParentLayer.AttachParticleEffect(BigExplosionParticleEffect);
#endif
                    break;
				...
				#if WINDOWS_PHONE
                    GokuPowerParticleEffect = GameDirector.Instance.CurrentResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.GokuPower, true);
                    GokuPowerParticleEffect.Initialise();
                    ParentLayer.AttachParticleEffect(GokuPowerParticleEffect);
#endif
                    break;
            ...
            switch (CurrentSpecialType)
            {
                case SpecialTypes.None:
                    break;
                case SpecialTypes.Bomb:
                    Texture[] animationTextures = new GameLibrary.UI.Texture[] 
                    {
                ...
                case SpecialTypes.MagicBomb:
                    break;
                case SpecialTypes.Goku:
                    animationTextures = new GameLibrary.UI.Texture[] 
                    {
        ...
        {
#if WINDOWS_PHONE
            MagicTrailParticleEffect = GameDirector.Instance.GlobalResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.MagicTrail, true);
            MagicTrailParticleEffect.Initialise();
            ParentLayer.AttachParticleEffect(MagicTrailParticleEffect);

            SmokePoofParticleEffect = GameDirector.Instance.CurrentResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.SmokePoof, true);
            SmokePoofParticleEffect.Initialise();
            ParentLayer.AttachParticleEffect(SmokePoofParticleEffect);

            BubbleExplosionEffect = GameDirector.Instance.CurrentResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.MagicBomb, true);
            BubbleExplosionEffect.Initialise();
            ParentLayer.AttachParticleEffect(BubbleExplosionEffect);

            StarsExplosionEffect = GameDirector.Instance.CurrentResourcesManager.GetCachedParticleEffect((int)GameDirector.ParticleEffects.Bubbles, true);
            StarsExplosionEffect.Initialise();
            ParentLayer.AttachParticleEffect(StarsExplosionEffect);
#endif
        }
        ...
        private void SetupTimers()
        {
            ...
            goldenHamstaAnimationTimer.OnUpdate += (sender, args) =>
            {
                if (AbsolutePosition.Y < CurrentLevel.HUDInfoLayer.CurrentLevelText.AbsolutePosition.Y + GlobalConstants.BlockSize)
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        StarsExplosionEffect.Trigger(CurrentLevel.HUDInfoLayer.CurrentLevelText.AbsolutePosition + new Vector2(0, GlobalConstants.BlockSize / 2));
                    }
        ...
        private void SetupTouchHandlers()
        {
            OnTouchDown += (sender, pos) =>
            {
                if (State == States.Idle)
                {
                    ...
                    // Each time the user starts dragging a block, the current combo is stopped
            ...
            OnDragging += (sender, touchPosition) =>
            {
                if (State == States.Dragging && touchPosition != LastDraggingPosition)
                {
                    ...
                    //Lock block inside the limits of the board. (-16 offset of MaxX.)
                    ...
                    if (LevelBoardController.BlocksMiniPhysicsEngine.GetClosestLineIntersection(currentPosX, currentPosY, out hCollisionPoint, out vCollisionPoint, this, 5, out hCollisionBlock, out vCollisionBlock))
                    {
                        ...
                        //+6 FOR HORIZONTAL positions and + 3 for vertical positions. this is the difference between the center of CollisionArea and the AbsolutePosition. 
...
#if WINDOWS_PHONE
                    MagicTrailParticleEffect.Trigger(AbsolutePosition);
#endif
                }
            };
        ...
        /// <summary>
        /// Verify if column or row are changed and free/check the board position.
        /// </summary>
        ...
        /// <summary>
        /// Do not change this method defines the default collision used for example by the touch panel manager. 
        /// If you want to use the collision area with a factor of 1 for example, just use the method overloading!
        /// </summary>
        /// <returns></returns>
            ...
            if (State != States.Disposed)
            {
                ...
                if (CurrentSpecialType == SpecialTypes.None)
                {
                    ...
                    else if (Type == BlockTypes.GoldenBlock)
                    {
                        ...
                        BubbleExplosionEffect.Trigger(AbsolutePosition);
...
#if WINDOWS_PHONE
                        switch (removalEffect)
                        {
                            case RemovalEffectEnum.SimpleExplosion:
                                SmokePoofParticleEffect.Trigger(AbsolutePosition);
                                break;

                            case RemovalEffectEnum.RainbowExplosion:
                                BubbleExplosionEffect.Trigger(AbsolutePosition);
                                break;

                            case RemovalEffectEnum.StarsExplosion:
                                StarsExplosionEffect.Trigger(AbsolutePosition);
                                break;
...
#if WINDOWS_PHONE
                            BigExplosionParticleEffect.Trigger(AbsolutePosition);
#endif
...
#if WINDOWS_PHONE
                            GokuPowerParticleEffect.Trigger(AbsolutePosition);
#endif
        ...
        {
#if WINDOWS_PHONE
            ParentLayer.DetachParticleEffect(BigExplosionParticleEffect);
            ParentLayer.DetachParticleEffect(SmokePoofParticleEffect);
            ParentLayer.DetachParticleEffect(MagicTrailParticleEffect);
            ParentLayer.DetachParticleEffect(GokuPowerParticleEffect);
            ParentLayer.DetachParticleEffect(StarsExplosionEffect);
            ParentLayer.DetachParticleEffect(BubbleExplosionEffect);
#endif
        ...
        /// <summary>
        /// Test if given flags has the given flag.
        /// </summary>
        ...
        private void UpdateOnFalling(TimeSpan elapsedTime)
        {
            ...
            {
                //+6 FOR HORIZONTAL positions and + 3 for vertical positions. this is the difference between the center of CollisionArea and the AbsoulutePosition. 
        ...
        private ParticleEffect MagicTrailParticleEffect { get; set; }
        private ParticleEffect SmokePoofParticleEffect { get; set; }
        private ParticleEffect BigExplosionParticleEffect { get; set; }
        private ParticleEffect GokuPowerParticleEffect { get; set; }
        private ParticleEffect BubbleExplosionEffect { get; set; }
        private ParticleEffect StarsExplosionEffect { get; set; }
