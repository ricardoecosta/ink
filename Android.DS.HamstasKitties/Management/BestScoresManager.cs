    ...
    public class BestScoresManager : IManager
    {
        ...
        public void RequestTotalSync()
        {
            ...
            if (NetworkManager.Instance.IsNetworkAvailable && needsRequest)
            {
                ...
                ScoreloopService.Instance.GetCurrentUserRank((int)GameDirector.GameModes.Countdown, ScoreloopService.LeaderboardTimeScopes.Overall);
                ScoreloopService.Instance.GetCurrentUserRank((int)GameDirector.GameModes.GoldRush, ScoreloopService.LeaderboardTimeScopes.Overall);
                ScoreloopService.Instance.GetCurrentUserRank((int)GameDirector.GameModes.ChillOut, ScoreloopService.LeaderboardTimeScopes.Overall);
#endif
            }
        }
        ...
        private void SyncLocalBestScore(GameDirector.GameModes mode)
        {
            ...
            {
#if WINDOWS_PHONE
                if (ScoreloopService.Instance.SubmitScore(localScore.Score, (int)mode, true))
                {
        ...
        private void OnUserLastScoreReceivedHandler(Score score)
        {
            if (score != null)
            {
                SyncBestScore((GameDirector.GameModes)score.Mode, score.ScoreResult);
            }
        }
